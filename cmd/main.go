...
import (
	...
	"github.com/nikhil-thomas/Resource-Pool/internal/controller"
	"github.com/nikhil-thomas/Resource-Pool/internal/lease"
	"github.com/nikhil-thomas/Resource-Pool/internal/provider"
	"github.com/nikhil-thomas/Resource-Pool/internal/provider/snowflake"
...
func main() {
	...
	flag.StringVar(&namespace, "namespace", "default",
		"The namespace where leases and provider configurations are managed.")
	opts := zap.Options{
		Development: true,
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "48cc0496.dataverse.redhat.com",
	...
	snowflakeProvider := snowflake.New(mgr.GetClient(), mgr.GetAPIReader())
	if err := providerRegistry.Register(snowflakeProvider); err != nil {
		setupLog.Error(err, "Failed to register snowflake provider")
		os.Exit(1)
	}

	// TODO: Register additional providers here
	// Example:
	// postgresProvider := postgres.New(mgr.GetClient())
	...
	for _, providerName := range providerRegistry.List() {
		...
		resources, err := prov.Initialize(ctx, namespace)
		if err != nil {
			setupLog.Error(err, "Failed to initialize provider", "provider", providerName)
			os.Exit(1)
		}
		setupLog.Info("Provider initialized", "provider", providerName, "resourceCount", len(resources))

		providerRegistry.SetResources(providerName, resources)

		if err := leaseManager.InitializeLeasesForProvider(ctx, providerName, resources); err != nil {
			setupLog.Error(err, "Failed to initialize leases", "provider", providerName)
			os.Exit(1)
		}
		setupLog.Info("Leases initialized", "provider", providerName, "leaseCount", len(resources))
	}
